...
public class AccountService implements UserDetailsService {
    ...
    public Account save(Account account) {
        ...
        if (account.getPhoto() == null) {
            // String path = photoPrefix.replace("**", "images/photo.png");
            account.setPhoto(photoPrefix);// Set default user role for not defined role accounts
        }
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        for (Authority _auth : account.getAuthorities()) {
            grantedAuthority.add(() -> _auth.getAuthorityName());
        }
        return new User(
                account.getEmail(),
                account.getPassword(),
    ...
    public Optional<Account> getByEmail(String authUser) {
